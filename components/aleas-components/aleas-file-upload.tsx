...
const AleasFileUpload = (props: AleasFileUploadProps) => {
    ...
    return <form
        ...
        className={mergeClasses(
            "relative text-center w-[28rem] h-[16rem] max-w-full",
            className
        )}
    >
        <input id="input-file-upload" type="file" multiple={multiple} onChange={handleChange} className="hidden full" />
        <label htmlFor="input-file-upload" className={mergeClasses(
            "flex flex-col items-center justify-center full p-12",
            "cursor-pointer hover:underline hover:bg-slate-600/40 transition-colors duration-200",
            "border-2 rounded-2xl border-dashed border-green-500/70",
            dragActive ? "bg-slate-600/70" : ""
        )}>
            <svg viewBox="0 0 24 24" className=" w-12 h-12 fill-white mb-4">
        ...
        { dragActive && <div className="absolute top-0 left-0 full" onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop}></div> }
    </form>
  };
